...
export default function SkillToggle({ selectedSkills = [], setSelectedSkills }) {
  const { t } = useI18n();

  const toggleSkill = (skill) => {
    setSelectedSkills((previousSkills) => {
      const isSelected = previousSkills.includes(skill);

      if (isSelected && previousSkills.length === 1) {
  ...
  return (
    <div className="skills-toggle-container">
      <p className="text-xs text-terracotta mb-2">{t('select_skills')}</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {SKILLS.map((skill) => {
          const isSelected = selectedSkills.includes(skill.key);
          const isLastSelected = isSelected && selectedSkills.length === 1;

          return (
            <div key={skill.key} className="skill-toggle-wrapper">
              <div className="relative inline-flex items-center">
                ...
                <div className="toggle-container">
                  <input
                    ...
                    aria-labelledby={`skill-label-${skill.key}`}
                    checked={isSelected}
                    disabled={isLastSelected}
                    onChange={() => toggleSkill(skill.key)}
                  />
                  <span className="toggle-handle-wrapper" aria-hidden="true">
                    <span className="toggle-handle">
                      <span className="toggle-handle-knob" />
                      <span className="toggle-handle-bar-wrapper">
                        <span className="toggle-handle-bar" />
                      </span>
                    </span>
                  </span>
                  <label htmlFor={`skill-${skill.key}`} className="toggle-base" aria-hidden="true">
                    <span className="toggle-base-inside" />
                  </label>
                </div>
              </div>
              <span className={`skill-label ${isSelected ? 'skill-selected' : ''}`} id={`skill-label-${skill.key}`}>
                {skill.label}
              </span>
            </div>
          );
      ...
      <style>{UIVERSE_TOGGLE_STYLES}</style>
    </div>
  );
}
